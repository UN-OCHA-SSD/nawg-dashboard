...
export function Trend({model,selected,period,compare,onPeriod,countyPage=false}) {
  ...
  const gapSegments=useMemo(()=>trendGapSegments(data),[data]);
  ...
  return <section className={'trend-section '+(countyPage?'county-trend':'')} aria-label="County severity trend">
    <div className="trend-heading"><div><h2><span>{name}</span><span className="heading-divider">/</span>{countyPage?'Severity trend':'Needs severity over time'}</h2><p title="Dotted lines connect reported values across missing or conflicting months. They are visual guides, not estimated monthly scores.">{metricName} · dotted links span data gaps, not estimates</p></div><div className="trend-actions">
      ...
      <div className="segmented small">{[12,24].map(n=><button key={n} className={range===n?'active':''} onClick={()=>setRange(n)}>{n}m</button>)}</div>
    ...
    <div className="trend-canvas">{any?<ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{top:12,right:30,bottom:4,left:4}} onClick={s=>{const k=s?.activeLabel;if(model.periods.includes(k))onPeriod(k);}}>
      ...
      <Line type="linear" dataKey="score" stroke="var(--chart-line)" strokeWidth={2.5} dot={{r:3.5,strokeWidth:1.5,stroke:'var(--panel)',fill:'var(--chart-line)'}} activeDot={{r:6}} connectNulls={false} isAnimationActive={false}/>
